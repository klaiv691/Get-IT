...
const SUPABASE_ANON_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImtwcXpjbGV6YmVoeWdsdG5xZmF0Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODY1MzU3MDcsImV4cCI6MjEwMjExMTcwN30.aHSEUkOQBfKEaf0ZqgRKZii7FSB-IgVYtOCkbQk3GRA";
...
function ticketCardHtml(e, i) {
  return `
    <a class="ticket ${catClass(e.category)}" style="--i:${i || 0}" href="case-study.html?slug=${encodeURIComponent(e.slug)}">
      ${e.featured_image ? `<img class="ticket-image" src="${escapeHtml(storageUrl(e.featured_image))}" alt="${escapeHtml([e.device_manufacturer, e.device_model].filter(Boolean).join(" ") || e.title)}" loading="lazy">` : ""}
...
function groupChecklistByDomain(keys) {
  ...
  return Object.entries(VOCAB_DOMAINS)
    .map(([domain, domainKeys]) => ({
      domain,
      items: domainKeys.filter((k) => keys.includes(k)),
    }))
...
function scoreSimilar(current, candidates, limit) {
  ...
  return candidates
    ...
    .map((c) => {
      ...
      score += (c.framework_topics || []).filter((t) =>
        topics.includes(t),
      ).length;
    ...
    .sort(
      (a, b) =>
        b.score - a.score || new Date(b.entry.date) - new Date(a.entry.date),
    )
    .slice(0, limit)
    .map((x) => x.entry);
}
